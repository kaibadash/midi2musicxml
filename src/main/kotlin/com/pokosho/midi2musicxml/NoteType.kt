...
enum class NoteType(val type: String) {
    ...
    companion object {
        fun note2beat(): HashMap<NoteType, Double> {
            return hashMapOf(
                    NOTE_TYPE_64TH to 0.125,
                    NOTE_TYPE_32ND to 0.125,
                    NOTE_TYPE_16TH to 0.25,
                    NOTE_TYPE_EIGHTH to 0.5,
                    NOTE_TYPE_QUARTER to 1.0,
                    NOTE_TYPE_HALF to 2.0,
                    NOTE_TYPE_WHOLE to 4.0
            )
        }
        ...
        fun toNoteType(beat: Double): NoteType {
            // 近似値を探す
            val map = note2beat()
            val approximateBeat = map.values.stream().min(Comparator.comparing { x: Double ->
                Math.abs(x - beat)
            }).get()
            var key: NoteType? = null
            map.forEach { k, v ->
                if (approximateBeat == v) {
                    key = k
                    return@forEach
                }
            }
            return key ?: throw IllegalArgumentException("NoteType is not found against beat(${beat})")
        }
    }

    fun tick(): Double {
        return note2beat()[this] ?: throw IllegalArgumentException("NoteType is not set")
    }
